...
(function() {
  var Backbone, BackboneOrm, sqlr, _,
  ...
  (module || {}).exports = this.BackboneOrm = BackboneOrm = (function(_super) {
    ...
    BackboneOrm.prototype._hookHasMany = function(name, rel) {
      var k, klass, models, theirs,
        _this = this;
      klass = (k = rel.hasMany) instanceof Model ? k : k();
      theirs = rel.theirFk;
    ...
    BackboneOrm.prototype._hookHasManyVia = function(name, rel) {
      var k, klass, mine, models, theirs, via, viaKlass,
        _this = this;
      klass = (k = rel.hasMany) instanceof Model ? k : k();
      viaKlass = (k = rel.via) instanceof Model ? k : k();
      mine = rel.myViaFk;
    ...
    BackboneOrm.prototype._hookGroup = function(name, group) {
      ...
      klass = (k = this.relations[group[0]].hasMany) instanceof Model ? k : k();
  ...
  BackboneOrm.sync = function(method, model, options) {
    var cols, indexes, m, n, queries, set, table, vals, values, where;
    table = model.table;
    queries = [];
    values = [];
    indexes = options.compositeKey || model.compositeKey || ['id'];
    indexes = _.reduce(indexes, function(indexes, index) {
    ...
    return _.each(queries, function(query) {
      ...
      return app.db.query(query, function(err, result) {
        if (err) {
  ...
  sqlr = {
    columns: function(data, table) {
      return _.map(data, function(col, i) {
        if (!(table || _.isArray(data))) {
          col = i;
        }
        return sqlr.column(col, table);
      }).join(', ');
    },
    column: function(col, table) {
      return "" + (table ? "\"" + table + "\"." : '') + "\"" + col + "\"";
    },
    values: function(data, values) {
      return _.reduce(data, function(text, val) {
        var n;
        values.push(val);
        n = values.length;
    ...
    colsAreVals: function(data, values, table, delimiter) {
      if (delimiter == null) {
        delimiter = ', ';
      }
      return _.map(data, function(val, col) {
        return sqlr.binaryGet(sqlr.column(col, table), val, values);
      }).join(delimiter);
    },
    colsToVals: function(data, values, table) {
      return _.map(data, function(val, col) {
        return sqlr.binarySet(sqlr.column(col, table), val, values);
      }).join(', ');
    },
    binaryGet: function(left, right, values, operator) {
      var l, o, r;
      if (operator == null) {
        operator = '=';
      }
    ...
    binarySet: function(left, right, values) {
      values.push(right);
      right = "$" + values.length;
      return "" + left + " = " + right;
    }
